...
const cloudinaryConsoleId = await env('CLOUDINARY_CONSOLE_ID');
...
while (true) {
  ...
  const directories =
    lastSelection === actions.CREATE_NEW ? [] : await getFolders(chosenDirectory);
...
for (const image of images) {
  ...
  // clipboard history (which I recommend you get!)
...
function openFolder(folder) {
  ...
  console.log('opening');
  return exec(
    `open "https://cloudinary.com/console/${cloudinaryConsoleId}/media_library/folders/${encodedFolder}"`
  );
}
...
> Credit to [Kent Dobbs](https://github.com/johnlindquist/kit/discussions/310)
