...
[FTX](https://ftx.us/). I'm using the `ftx-api` npm module for making trades with an automated
trading bot.
...
import { FTXBalancesResponse, FTXMarket, FTXMarketResponse, FTXMarketsResponse } from './types';
...
const getUsdBalance = async () => {
  ...
  if (resp.success && resp.result.length && resp.result.findIndex((x) => x.coin === QUOTE) !== -1) {
