...
description: 'Template for building a lambda function with credentials stored in AWS Secrets Manager'
...
The goal of this project is to store credentials in AWS Secretes Manager and use them in a Lambda function.
To keep it simple, we're going to create a Cloudwatch event to trigger our function on a schedule.
The Lambda will then use the credentials for something super duper secret (connect to a database, use an external API, etc.).

<Image
  src='/images/lambda-with-secrets/architecture.png'
