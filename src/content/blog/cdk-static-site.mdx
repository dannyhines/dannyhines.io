...
10 minutes later, you'll have your app.
...
the code within `bin` and `lib`. Inside the `bin` directory you have the executeable(s) that
...
region you're deploying your resources to.
...
We start by making the hosted zone S3 bucket(s) for howmanyblimpsarethere.com and
www.howmanyblimpsarethere.com. We also need a TLS certificate for the url, and we need to
...
we made, that points to the disribution:
...
So let's go back to the React app and build the app:
...
And that's it! I bought the domain while watching the first quarter of a Sunday night NFL
game, and by halftime it was deployed. So like 45 minutes to deploy a website? Not bad.
